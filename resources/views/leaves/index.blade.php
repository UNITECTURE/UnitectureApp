...
@section('content')
            ...
            <main class="flex-1 overflow-y-auto p-4 lg:p-8">
                <div class="space-y-6">
                    <div class="flex items-center justify-between">
                        ...
                        <button onclick="document.getElementById('apply-leave-modal').classList.remove('hidden')"
                            class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg flex items-center gap-2 transition-colors shadow-md">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                        ...
                        <div style="background-color: #2563EB;" class="rounded-lg p-5 shadow-md">
                            <div class="flex items-center justify-between mb-3">
                                <div>
                                    <p style="color: #FFFFFF;" class="text-xs font-bold mb-2 uppercase tracking-wider">
                                        Available Balance</p>
                                    <p style="color: #FFFFFF;" class="text-3xl font-bold">
                                        {{ number_format($earnedLeaves - $usedLeaves, 1) }}</p>
                                </div>
                            ...
                            <div class="flex items-center justify-between gap-4 pt-3 border-t border-white/20">
                                <div>
                                    ...
                                    <p style="color: #FFFFFF;" class="text-sm font-semibold">
                                        {{ number_format($earnedLeaves, 1) }}</p>
                                </div>
                                <div>
                                    <p style="color: rgba(255,255,255,0.7);" class="text-xs">Used</p>
                                    <p style="color: #FFFFFF;" class="text-sm font-semibold">
                                        {{ number_format($usedLeaves, 1) }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <button @click="filterStatus = 'all'"
                            class="relative bg-white rounded-lg shadow border border-slate-100 overflow-hidden cursor-pointer hover:border-blue-300 hover:shadow-md transition-all"
                            :class="filterStatus === 'all' && 'border-blue-500 shadow-md'">
                            <div class="p-5 flex items-start justify-between gap-3">
                                <div class="h-1 bg-blue-500 absolute top-0 left-0 right-0"></div>
                                <div>
                                    <p class="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-2">Total
                                        Requests</p>
                        ...
                        <button @click="filterStatus = 'rejected'"
                            class="relative bg-white rounded-lg shadow border border-slate-100 overflow-hidden cursor-pointer hover:border-red-300 hover:shadow-md transition-all"
                            :class="filterStatus === 'rejected' && 'border-red-500 shadow-md'">
                            <div class="p-5 flex items-start justify-between gap-3">
                                <div class="h-1 bg-red-500 absolute top-0 left-0 right-0"></div>
                                <div>
                                    <p class="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-2">Rejected
                                    </p>
                        ...
                        <button @click="filterStatus = 'approved'"
                            class="relative bg-white rounded-lg shadow border border-slate-100 overflow-hidden cursor-pointer hover:border-green-300 hover:shadow-md transition-all"
                            :class="filterStatus === 'approved' && 'border-green-500 shadow-md'">
                            <div class="p-5 flex items-start justify-between gap-3">
                                <div class="h-1 bg-green-500 absolute top-0 left-0 right-0"></div>
                                <div>
                                    <p class="text-xs text-slate-500 uppercase tracking-wider font-semibold mb-2">Approved
                                    </p>
                                    <p class="text-3xl font-bold text-green-600">
                                        {{ $leaves->whereIn('status', ['approved', 'approved_by_superadmin'])->count() }}
                                    </p>
                                </div>
                                <div
                            ...
                            <table class="w-full text-sm text-left">
                                <thead class="bg-slate-50 border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 font-semibold text-slate-700 text-center">Approval Progress
                                        </th>
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @forelse($leaves as $leave)
                                        <tr class="hover:bg-slate-50/50 transition-colors"
                                            x-show="filterStatus === 'all' || (filterStatus === 'rejected' && '{{ $leave->status }}' === 'rejected') || (filterStatus === 'approved' && ['approved', 'approved_by_superadmin'].includes('{{ $leave->status }}'))">
                                            {{-- Leave Type / Reason --}}
                                            <td class="px-6 py-5">
                                                <p class="font-semibold text-slate-900" title="{{ $leave->reason }}">
                                                    {{ Str::limit($leave->reason, 35, '...') }}</p>
                                                <p class="text-xs text-slate-400 capitalize">{{ $leave->leave_type }} Leave</p>
                                            </td>

                                            {{-- Dates & Duration --}}
                                            <td class="px-6 py-5">
                                                <p class="text-slate-900 font-medium">{{ $leave->start_date->format('d M') }} -
                                                    {{ $leave->end_date->format('d M') }}</p>
                                                ...
                                                <div class="flex items-center justify-center">
                                                    <div class="inline-flex items-center justify-center gap-2">
                                                        ...
                                                        @if($isEmployee)
                                                            ...
                                                            <div class="flex flex-col items-center gap-0.5">
                                                                <div
                                                                    class="w-7 h-7 rounded-full {{ $supervisorDone ? 'bg-green-500' : ($supervisorRejected ? 'bg-red-500' : 'bg-slate-200') }} flex items-center justify-center">
                                                                    @if($supervisorDone)
                                                                        <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24"
                                                                            stroke="currentColor">
                                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                ...
                                                                <span
                                                                    class="text-[10px] font-semibold text-slate-700">Supervisor</span>
                                                            </div>

                                                            {{-- Admin --}}
                                                            <div class="flex flex-col items-center gap-0.5">
                                                                <div
                                                                    class="w-7 h-7 rounded-full {{ $adminDone ? 'bg-green-500' : ($adminRejected ? 'bg-red-500' : 'bg-slate-200') }} flex items-center justify-center">
                                                                    @if($adminDone)
                                                                        <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24"
                                                                            stroke="currentColor">
                                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                        ...
                                                        @elseif($isStaff)
                                                            ...
                                                            <div class="flex flex-col items-center gap-0.5">
                                                                <div
                                                                    class="w-7 h-7 rounded-full {{ $superadminDone ? 'bg-green-500' : ($superadminRejected ? 'bg-red-500' : 'bg-slate-200') }} flex items-center justify-center">
                                                                    @if($superadminDone)
                                                                        <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24"
                                                                            stroke="currentColor">
                                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                                ...
                                                                <span class="text-[10px] font-semibold text-slate-700">Super
                                                                    Admin</span>
                                                            </div>
                                                        @endif
                                                    </div>
                                                ...
                                                <div class="flex items-center justify-center gap-4">
                                                    <div>
                                                        @if($leave->status === 'pending')
                                                            <span style="background-color: #FB923C;"
                                                                class="text-white text-xs font-bold py-1.5 px-3 rounded-full">Pending</span>
                                                        @elseif($leave->status === 'approved' || $leave->status === 'approved_by_superadmin')
                                                            <span style="background-color: #4ADE80;"
                                                                class="text-white text-xs font-bold py-1.5 px-3 rounded-full">Approved</span>
                                                        @elseif($leave->status === 'approved_by_supervisor')
                                                            <span style="background-color: #60A5FA;"
                                                                class="text-white text-xs font-bold py-1.5 px-3 rounded-full">Pending
                                                                Admin</span>
                                                        @elseif($leave->status === 'rejected')
                                                            <span style="background-color: #F87171;"
                                                                class="text-white text-xs font-bold py-1.5 px-3 rounded-full">Rejected</span>
                                                        @elseif($leave->status === 'cancelled')
                                                            <span
                                                                class="bg-slate-300 text-slate-800 text-xs font-bold py-1.5 px-3 rounded-full">Cancelled</span>
                                                        @else
                                                            <span
                                                                class="bg-slate-200 text-slate-800 text-xs font-bold py-1.5 px-3 rounded-full">{{ ucfirst(str_replace('_', ' ', $leave->status)) }}</span>
                                                        @endif
                                                    </div>
                                                    ...
                                                    @if($showCancel)
                                                        <form action="{{ route('leaves.cancel', $leave) }}" method="POST"
                                                            ...
                                                            @csrf
                                                            <button type="submit"
                                                                class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-red-500 hover:bg-red-600 shadow-sm transition-colors"
                                                                title="Cancel Leave" aria-label="Cancel Leave">
                                        ...
                                        <tr>
                                            <td colspan="4" class="px-6 py-12 text-center">
                                                <div class="flex flex-col items-center justify-center">
                                                    ...
                                                    <p class="text-slate-400 text-sm mt-1">Start by applying for a leave using
                                                        the button above.</p>
                                                </div>
                                            </td>
                                        </tr>
    ...
    <div id="apply-leave-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden">
        <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity"
            onclick="document.getElementById('apply-leave-modal').classList.add('hidden')"></div>
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl z-10 mx-4 overflow-hidden">
            <div style="background: linear-gradient(135deg, #2563EB 0%, #1d4ed8 100%);"
                ...
                <button onclick="document.getElementById('apply-leave-modal').classList.add('hidden')"
            ...
            <form action="{{ route('leaves.store') }}" method="POST" class="p-8 space-y-6">
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date" required min="{{ date('Y-m-d') }}"
                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm focus:outline-none transition-colors">
                    </div>
                    <div>
                ...
                <div style="background-color: #EFF6FF; border-color: #BFDBFE;" class="border rounded-lg p-4">
                    <p class="text-blue-900 text-sm"><strong>Note:</strong> Leave type (Paid/Unpaid) will be determined
                        automatically based on your leave balance.</p>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <label
                            class="flex items-center gap-3 p-4 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                            <input type="radio" name="leave_category" value="planned" required
                        ...
                        <label
                            class="flex items-center gap-3 p-4 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                            <input type="radio" name="leave_category" value="emergency" required
                                class="w-4 h-4 text-blue-600 cursor-pointer">
                            <div>
                ...
                <div>
                    ...
                    <textarea name="reason" rows="3" required placeholder="e.g. Emergency, Medical, Family function"
                        class="w-full px-4 py-2.5 rounded-lg border border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm focus:outline-none transition-colors resize-none"></textarea>
                </div>

                <div class="flex items-center justify-end pt-4 border-t border-slate-100">
                    <div class="flex items-center gap-3">
                        <button type="button" onclick="document.getElementById('apply-leave-modal').classList.add('hidden')"
                            class="px-6 py-2.5 border border-slate-300 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors">Cancel</button>
                        <button type="submit"
                            class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold transition-colors">Submit
                            Request</button>
                    </div>
                </div>
            </form>
