                ...
                <div class="container mx-auto px-6 py-8">
                    <div class="mb-8 flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-slate-400 text-sm mt-1 font-medium">View all users. Manage teams from the Teams
                                page.</p>
                        </div>
                        <div class="flex items-center gap-3">
                            <a href="{{ route('teams.index') }}"
                                class="px-4 py-2 rounded-lg border border-indigo-200 text-indigo-700 text-sm font-medium hover:bg-indigo-50">Teams</a>
                            <a href="{{ route('users.create') }}"
                                class="px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700">Add
                                User</a>
                    ...
                    @if(session('success'))
                        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 2000)"
                            ...
                            class="mb-6 p-4 rounded-lg bg-green-50 border border-green-200 text-green-800 text-base font-medium">
                            {{ session('success') }}</div>
                    @endif
                    @if(session('error'))
                        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 2000)"
                            x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100"
                            x-transition:leave-end="opacity-0"
                            class="mb-6 p-4 rounded-lg bg-red-50 border border-red-200 text-red-800 text-base font-medium">
                            {{ session('error') }}</div>
                    @endif
                            ...
                            <table class="w-full text-left">
                                <thead class="bg-slate-50 border-b border-slate-100">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase">Primary
                                            Supervisor</th>
                                        <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase">Secondary
                                            Supervisor</th>
                                ...
                                <tbody>
                                    @foreach($users as $u)
                                        <tr class="border-b border-slate-50 hover:bg-slate-50/50">
                                            ...
                                            <td class="px-4 py-3 text-sm text-slate-600">{{ ucfirst($u->role->name ?? '—') }}
                                            </td>
                                            <td class="px-4 py-3 text-sm text-slate-600">
                                                {{ $u->primarySupervisor ? $u->primarySupervisor->full_name : '—' }}</td>
                                            <td class="px-4 py-3 text-sm text-slate-600">
                                                {{ $u->secondarySupervisor ? $u->secondarySupervisor->full_name : '—' }}</td>
                                            <td class="px-4 py-3">
                                                <div class="flex items-center gap-4">
                                                    ...
                                                    @if($u->id !== Auth::id())
                                                        <form action="{{ route('users.destroy', $u->id) }}" method="POST"
                                                            class="inline"
                                                            onsubmit="return confirm('Delete {{ $u->full_name }}? This cannot be undone.');">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
