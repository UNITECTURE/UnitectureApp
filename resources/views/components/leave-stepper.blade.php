...
@php
    $steps = [
        ...
        'supervisor' => ['label' => 'Supervisor', 'status' => 'pending'],
        'admin' => ['label' => 'Admin', 'status' => 'pending'],
    ];
    ...
    if ($status === 'approved_by_supervisor') {
        ...
        $steps['supervisor']['status'] = 'complete';
        $steps['admin']['status'] = 'complete';
    } elseif ($status === 'rejected') {
        // Handle rejection visualization if needed, or just show where it stopped
        // For now, let's assume rejected means the flow stops.
    }
@endphp
...
<div class="w-full py-4">
    <div class="relative flex items-center justify-between w-full">
        ...
        <div class="absolute left-0 top-1/2 transform -translate-y-1/2 h-1 bg-blue-500 transition-all duration-500 -z-10"
             style="width: {{ $status === 'approved' ? '100%' : ($status === 'approved_by_supervisor' ? '50%' : '0%') }}"></div>
        ...
        <div class="flex flex-col items-center bg-white px-2">
            ...
            <span class="text-xs font-medium mt-1 text-slate-600">Supervisor</span>
        </div>

        <!-- Admin Step -->
        <div class="flex flex-col items-center bg-white px-2">
            <div class="w-8 h-8 rounded-full flex items-center justify-center {{ $steps['admin']['status'] === 'complete' ? 'bg-green-500 text-white' : 'bg-slate-200 text-slate-500' }}">
                 @if($steps['admin']['status'] === 'complete')
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                @else
                    <span class="text-xs">3</span>
                @endif
            </div>
            <span class="text-xs font-medium mt-1 text-slate-600">Admin</span>
        </div>
    </div>
</div>
